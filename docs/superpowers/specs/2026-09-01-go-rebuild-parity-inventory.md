...
- **Status line** (mode-dependent): deps — elided missing-module list + y/n/esc hints; input — live text w/ cursor block; confirm — message + y/esc; default — running-script line with 10-cell eighth-block-precision ETA bar (`% · ~time left` or `+time over`) or transient status message (color by kind, fades over 5.2–6.3s window) or selected script's description+schedule hint.
