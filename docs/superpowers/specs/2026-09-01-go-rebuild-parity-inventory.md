...
> **2026-09-02:** migration machinery removed — fresh-install cutover chosen by the owner. Every item below was ported to Go and shipped (see the phase table), then deleted outright once the owner decided against ever transitioning an existing PowerShell-era install. This report is not rewritten to match — it documents what PS/early-Go did; the removal itself lives in the `go-rewrite` git history and `.superpowers/sdd/cutover-removal-report.md`.

1. *(retired 2026-09-02 — data-dir migration removed with `internal/migrate`/config's `~/.psscripts` adoption; a fresh install never has a `~/.psscripts` to find. The number stays so 2/3/4 do not move.)*
2. *(retired 2026-09-02 — legacy crontab marker recognition removed from `internal/cron`; only the current `# >>> scriptorium managed block …` markers are read or written now. The number stays so 3/4 do not move.)*
3. *(retired 2026-09-02 — `Update-StoRepoLayout`'s Go port, `scripts.MigrateLayout`, deleted along with its `Sync` wiring; a fresh install never has a root-level single-repo clone to migrate. The number stays so 4 does not move.)*
4. *(retired 2026-09-02 — install.sh's `$PSSCRIPTS_APP_DIR` fallback, the pre-rename repo-URL detect/repoint/reset path, the `~/.local/bin/psscripts` cleanup, the `psscripts.ps1` shim, and the psscripts-mcp systemd retirement (§13.2, §11.10) are all gone; checkout-mode git handling is now just `fetch` + `pull --ff-only`, NOTE-and-continue on failure.)*
...
*(2026-09-02: the legacy `psscripts-mcp.service` retirement step described below is removed — see §3.13.)*
...
*(2026-09-02: the `$PSSCRIPTS_APP_DIR` fallback, the pre-rename repo-URL detect/repoint/reset, and the legacy launcher cleanup described below are removed — see §3.13.)*
*(2026-09-03, v1.1.0: the prerequisite installs described below are BACK — the Go installer's v1.0 "warn about pwsh, never apt" stance is reversed by owner directive; the git auto-install alone stays retired (checkout mode dies with a message instead — you cannot be standing in a git checkout without git). The PATH check now APPENDS to the shell rc, marker-guarded, where this section says "warns (doesn't modify shell rc files)". Re-runs print `updated scriptorium vOLD → vNEW`. See divergence 31.)*
...
*(2026-09-02: the shim file itself is deleted from the tree — see §3.13.)*
...
23. **`theme` is a Go-only config key.** Selects a registered palette (night-owl default, catppuccin/gruvbox/tokyo-night alternates); PS has no such key and would print its unknown-key warning on a shared config, so the key is documented as post-cutover-only. Unknown value → warning + Night Owl. *Correction (2026-09-03, v1.0.1):* the key now resolves curated-first, then a `terminal` palette (entry 27), then any of the ~340 bubbletint IDs (entry 28), accepting both `-` and `_` spellings; the unknown-value warning names the three nearest known names instead of dumping the registry, and `]`/`[` cycle the full set live (session-only). The v1.0.0 wording above described only the four curated names.
...
26. **§12.2's focused-border role is folded onto Accent (Magenta), not Blue.** The design's semantic-token contract is thirteen tokens (`Bg Fg Muted Border Accent Success Warning Danger Info SelBg CardBg RuntimePS RuntimePy`), and §12.2 asks two different roles for a colour that has to share one: `borders=Border(Blue focused)` and `key-hints=Magenta`. Both land on `Accent`, which Night Owl fills with Magenta (`#c792ea`), so a focused pane's rule and title read in the same voice as the key hints — "you are here / press this" — and Blue (`#82aaff`) is left meaning exactly one thing in the UI, the `ps` runtime tag. Every other §12.2 role is honoured hex-for-hex from `Core.psm1` (success=Green, failure/missed=Red, killed/timeout/skipped=BrYellow, scheduled/queued=Cyan, python=Yellow, powershell=Blue, muted=BrBlack, selection=SelBg, zebra ground=CardBg). The only Core.psm1 colour left with no role is BrCyan (`#7fdbca`); White survives as CardBg's blend target. Observable effect: a focused pane border is Magenta where the PS TUI drew Blue. *Correction (2026-09-03, v1.0.1):* this fold was the root of the release's "purple voice" defect — Magenta landed on every border, hint key, selection bar and tab, out-shouting the data. The token contract is now FIFTEEN tokens: `Primary` (Blue `#82aaff`: focus borders, selection accent, hint keys, active tab, input cursors) and `Pulse` (BrCyan `#7fdbca`: spinner, focused-pane title) restore the PS Blue↔BrCyan interplay this entry had folded away, and `Accent` (Magenta) retreats to the brand chip and the palette overlay's highlight. BrCyan is no longer role-less; a focused pane border is Blue again, exactly as the PS TUI drew it. Same wave: Border tokens are pinned just above 3:1 against Bg (Night Owl's darkened, the alternates' lifted) and Muted tokens meet 4.5:1 (Night Owl `#637777` → `#708284`), both proven by computed WCAG ratios in the theme tests.
27. **The `terminal` palette maps the token contract onto ANSI 0-15 + the terminal's default fg/bg — and paints no ground, by design.** (Added 2026-09-03, v1.0.1.) Every other palette paints a full-frame Fg-on-Bg ground (the v1.0.1 fix for frames resetting to terminal defaults); `terminal` deliberately does not — its `Bg`/`Fg`/`CardBg` tokens are empty ("use the terminal's own default"), its remaining tokens are ANSI indices (Primary=12, Pulse=14, Accent=13, Success=2, Warning=3, Danger=1, Info=6, Muted/Border/SelBg=8, RuntimePS=4, RuntimePy=11), and no truecolor SGR is ever emitted, so the UI inherits the user's scheme wholesale, light terminals included. PS had no counterpart (its palette was embedded and always painted). Contrast floors do not apply: the actual colors belong to the terminal.
28. **The bubbletint adapter derives Border, CardBg and a Muted lift; the curated palettes hand-pin the same rules.** (Added 2026-09-03, v1.0.1.) Any of bubbletint v2.0.2's ~340 schemes fills the tokens via `fromTint`: Bg/Fg/SelectionBg direct; Green→Success, BrightYellow→Warning, Red→Danger, Cyan→Info, Blue→Primary, BrightCyan→Pulse, Purple→Accent, BrightBlack→Muted, Blue→RuntimePS, Yellow→RuntimePy. Where it diverges from curated behavior: Border is DERIVED (the quietest Bg-toward-Fg shade clearing 3:1) rather than taken from the scheme; CardBg blends 4.5% toward white on dark schemes but toward black on light ones; Muted is always lifted to 4.5:1 against Bg even when the scheme provides BrightBlack (most schemes' BrightBlack fails as text — Dracula's rates 2.2:1); and nil fields fall back along documented chains (Bg/Fg → black/white poles by `Dark`, SelectionBg → Bg+18% toward Fg, BrightYellow → Yellow, BrightCyan → Cyan → Blue, Purple → Blue, status colors → Fg). Curated palettes bake the identical Border/Muted floors into their pinned hexes instead of deriving at load. bubbletint is generated static data with an empty dependency closure — a test asserts no net/net-http/os-exec anywhere below it.
29. **The TUI frame grammar is the v1.1.0 rounded panel system above 100 columns; the v1.0 "rules, not boxes" grammar survives only as the floor.** (Added 2026-09-03, v1.1.0.) §1.12's PS panels drew full box-drawing frames with inset titles; v1.0.0 replaced them with bare top rules to protect the 80-column budget. v1.1.0 restores full frames — rounded `╭╮╰╯`, title inset in the top border, and (new, no PS counterpart) the pane's own key hints inset in the bottom border, rendered from the same `key.Binding` set the footer and help read (entry 24's per-view hint-list invariant stands: the tail renders the view-owned subset of the SAME list, never a second one). Below 100 columns every view keeps the v1.0.1 rule grammar (the floor's column/row budget); overlays are rounded boxes at every size. ASCII profiles draw `+-|`. Ground continuity, the selection-inside-the-border seam, scrollbar-inside-the-frame and wrap-accounts-for-the-frame are all test-pinned.
30. **The animation engine is one coalesced 16 ms clock; the spinner is 8 braille frames at 80 ms; the status fade interpolates per-frame in truecolor.** (Added 2026-09-03, v1.1.0.) §12.10's PS model (16 ms budget while animating, idle renders nothing, anim-only frames patch exactly the changed rows) is now matched more closely than v1.0.0 did: ONE self-rescheduling 16 ms `tea.Tick` drives spinner, marquee, fade, ETA motion and the activity pulse, armed only while something moves — the v1.0 standalone marquee (165 ms) and fade (100 ms) ticks are dead, their steppers evaluated frame-accurately from the injected clock. Observable divergences from the PS-era statements: the spinner drops §1.12's last two frames (`⠇⠏`) — 8 glyphs stepped at 80 ms boundaries rather than 10 at 10 fps; the status fade is a continuous per-frame Fg→Bg interpolation under truecolor where PS stepped through a fixed ramp (256/16-color profiles keep a 5-stop stepped fallback, and ANSI-basic palettes never interpolate — no per-frame SGR churn below truecolor); the ETA bar keeps §1.12's eighth-block sub-cell precision at 12 cells and adds a 300 ms ease toward its target; and the live-activity title breathes Pulse↔Muted on a ~2 s period while anything runs (lazygit-style; no PS counterpart, paneled frames only — the floor's card title stays static).
31. **install.sh installs the full prerequisite set again — the v1.0 pwsh-optional stance is reversed (owner directive, 2026-09-03).** The v1.0 Go installer warned about a missing `pwsh` instead of installing it ("apt/dpkg root access is no longer this script's business"). v1.1.0 restores §13.1's installs on apt systems: PowerShell 7 via the Microsoft repo (packages-microsoft-prod.deb for `/etc/os-release`'s `$VERSION_ID` → `dpkg -i` → `apt-get update` → `apt-get install -y powershell`) and `python3 python3-venv python3-pip` behind the real `python3 -m venv --help` check — through a privilege ladder §13.1 never needed (it assumed sudo): root runs apt directly, then `sudo -n`, then one interactive sudo prompt on a real tty, else a per-package WARN with the exact manual command; a failed prerequisite never aborts the install. Two further §13.1 divergences: the PATH check appends a marker-guarded `export PATH="$HOME/.local/bin:$PATH"` to the `$SHELL`-selected rc (creating it if missing, falling back to the old warning if unwritable) where §13.1 explicitly did not modify rc files; and re-runs print `updated scriptorium vOLD → vNEW` / "already current — binary refreshed" by asking the old and new binaries `--version`. The git auto-install stays retired: checkout mode requires git by construction and now says so instead of escalating.
