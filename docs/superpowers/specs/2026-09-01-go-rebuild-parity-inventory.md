...
18. **Shipping-window stubs (informational, all temporary).** Until its owning phase lands, the Go binary answers honestly instead of silently diverging: bare invocation (TUI, P10) prints a not-yet-available message to stderr and exit 1. This entry deletes when P10 ships. (`--mcp`/`--install-mcp-service` shipped live in P9 — see §11.9-11.11 for the tool set and the systemd install flow, and §11.12/the design-spec addendum for the co-hosted REST API that landed alongside it; `--run`'s dependency auto-install shipped in P8 — cron migration for repos that gain new scripts is unblocked; see entry 20 for what's Go-only about that implementation.)
